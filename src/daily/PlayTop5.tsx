    ...
    <div className="h-full flex flex-col select-none">
      <header className="shrink-0 flex items-center gap-3 px-5 pt-5 pb-2 pr-14">
        ...
        <div className="min-w-0">
          <div className="text-[0.6rem] uppercase tracking-[0.3em] text-fg/40">Top 5 · from {partner}</div>
      ...
      {result ? (
        <div className="flex-1 min-h-0 overflow-y-auto px-6 pb-8 flex flex-col items-center gap-4 animate-fade-up">
          <div className="text-[0.65rem] uppercase tracking-[0.3em] text-fg/40 mt-2">{partner}'s real order was</div>
          <RevealLadder puzzle={result} />
          <div className="font-display text-2xl font-bold text-accent text-center">
            {outcome(result.exact!, result.near!)}
          </div>
          <button onClick={onClose} className="mt-1 w-full max-w-sm min-h-[52px] rounded-xl border-2 border-fg/20 font-bold uppercase tracking-widest active:translate-y-px">
            Done
          </button>
        </div>
      ) : (
        ...
        return (
          <div
            ...
            className={
              ...
              (tier === 'exact' ? 'border-accent bg-accent/10' : tier === 'near' ? 'border-accent/40' : 'border-fg/15')
            }
          >
            <span className="w-6 shrink-0 text-xl font-bold tabular-nums text-accent">{i + 1}</span>
            <span className="flex-1 min-w-0 truncate text-sm uppercase tracking-wide text-left">{puzzle.items[itemIndex]}</span>
