    ...
    <div className="h-full flex flex-col select-none">
      <header className="shrink-0 flex items-center gap-3 px-5 pt-5 pb-2 pr-14">
        ...
        <div className="min-w-0">
          <div className="text-[0.6rem] uppercase tracking-[0.3em] text-fg/40">Their Numbers · from {partner}</div>
          <div className="text-lg font-bold leading-tight">{partner}'s numbers</div>
        </div>
      </header>
      <div className="flex-1 min-h-0 overflow-y-auto px-5 pb-6">
        {result ? (
          <div className="flex flex-col gap-3 animate-fade-up">
              ...
              return (
                <div
                  ...
                  className={
                    ...
                    (mark === 'exact' ? 'border-accent bg-accent/10' : mark === 'close' ? 'border-accent/40' : 'border-fg/15')
                  }
                >
                  <div className="text-sm leading-snug">{q}</div>
                  <div className="mt-1 flex items-baseline justify-between gap-3">
                    <span className="font-display text-3xl font-bold tabular-nums text-accent">{result.answers![i]}</span>
            ...
            <div className="font-display text-2xl font-bold text-accent text-center mt-1">{summary(result.marks!)}</div>
            <button onClick={onClose} className="w-full min-h-[52px] rounded-xl border-2 border-fg/20 font-bold uppercase tracking-widest active:translate-y-px">
              Done
            </button>
          </div>
        ) : (
