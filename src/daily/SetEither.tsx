...
import { PickForm } from './EitherKit'

// Picking your side of today's five, for real.
export function SetEither({
  partner,
  questions,
  onClose,
  forDate,
  ...
  const send = useCallback(async (picks: number[]) => {
    setBusy(true)
    setNote(null)
    try {
      await api.setEither(forDate ?? localDate(), questions, picks)
      setSent(true)
    } catch (e) {
    ...
    <div className="h-full flex flex-col select-none">
      <header className="shrink-0 flex items-center gap-3 px-5 pt-5 pb-2">
        ...
        <div className="min-w-0">
          <div className="text-[0.7rem] uppercase tracking-[0.22em] font-extrabold text-fg/50">{`This or That · ${forDate ? 'tomorrow' : 'today'}'s five`}</div>
          <div className="font-display text-xl font-extrabold leading-tight">Your picks</div>
        </div>
      </header>
      {sent ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8 text-center animate-fade-up">
          ...
          <div className="text-fg/60">
            {forDate ? `${partner} gets it tomorrow.` : `${partner} can predict them now.`}
          </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5 pb-6">
          <div className="text-sm text-fg/70 leading-snug mb-4">
            Pick your side of each — no fence-sitting. {partner} will be predicting them.
          </div>
          <PickForm questions={questions} onSubmit={(v) => void send(v)} label="Send them" busy={busy} p="A" />
